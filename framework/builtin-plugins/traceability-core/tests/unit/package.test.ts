...
import { businessFlowDefinitions, recordTraceabilityLink, publishCommonDimension, queueReconciliationItem, placeRecordOnHold, releaseRecordHold, amendRecord, reverseRecord } from "../../src/flows/catalog";
...
describe("plugin manifest", () => {
  it("keeps a stable package id and business contract surface", () => {
    expect(manifest.id).toBe("traceability-core");
    expect(manifest.kind).toBe("plugin");
    expect(manifest.publicCommands).toContain("traceability.links.record");
    expect(manifest.publicEvents).toContain("traceability.link-recorded.v1");
  });
});
...
describe("domain catalog", () => {
  it("keeps ERPNext parity references and operational surfaces visible", () => {
    ...
    expect(typeof recordTraceabilityLink).toBe("function");
    expect(typeof publishCommonDimension).toBe("function");
    expect(typeof queueReconciliationItem).toBe("function");
