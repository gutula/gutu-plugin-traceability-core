...
describe("traceability-core lifecycle integration", () => {
  const previousStateDir = process.env.GUTU_STATE_DIR;
  let stateDir = "";

  beforeEach(() => {
    stateDir = mkdtempSync(join(tmpdir(), "traceability-core-"));
  ...
  it("creates, advances, and reconciles a governed business record", async () => {
    const created = await createPrimaryRecord({
      ...
      actorId: "actor_admin",
      recordId: "traceability-core:demo",
      title: "Traceability & Dimensions Core Demo",
      ...
      correlationId: "traceability-core:corr",
      processId: "traceability-reconciliation:demo"
    });
    ...
    const reconciled = await reconcilePrimaryRecord({
      ...
      actorId: "actor_admin",
      recordId: "traceability-core:demo",
      exceptionId: "traceability-core:exception",
    ...
    const amended = await amendPrimaryRecord({
      ...
      actorId: "actor_admin",
      recordId: "traceability-core:demo",
      amendedRecordId: "traceability-core:demo:amended",
      expectedRevisionNo: 5,
      title: "Traceability & Dimensions Core Demo Amendment",
      reasonCode: "commercial-correction"
    });
    expect(amended.amendedRecordId).toBe("traceability-core:demo:amended");

    const reversed = await reversePrimaryRecord({
      tenantId: "tenant_demo",
      actorId: "actor_admin",
      recordId: "traceability-core:demo:amended",
      reversalRecordId: "traceability-core:demo:amended:reversal",
      expectedRevisionNo: 1,
      reasonCode: "cancel-amendment"
    });
    expect(reversed.reversalRecordId).toBe("traceability-core:demo:amended:reversal");
