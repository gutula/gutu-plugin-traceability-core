export type TraceabilityCoreSqliteOptions = {
  tablePrefix?: string;
};

export function buildTraceabilityCoreSqliteMigrationSql(options: TraceabilityCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "traceability_core_");
  return [
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS ${getTraceabilityCoreSqliteLookupIndexName(tablePrefix)} ON ${tablePrefix}primary_records (tenant_id, title, correlation_id);`,
    `CREATE INDEX IF NOT EXISTS ${getTraceabilityCoreSqliteStatusIndexName(tablePrefix)} ON ${tablePrefix}exception_records (tenant_id, status, severity);`
  ];
}

export function buildTraceabilityCoreSqliteRollbackSql(options: TraceabilityCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "traceability_core_");
  return [
...
export function getTraceabilityCoreSqliteLookupIndexName(tablePrefix = "traceability_core_"): string {
  return `${normalizePrefix(tablePrefix)}primary_lookup_idx`;
}

export function getTraceabilityCoreSqliteStatusIndexName(tablePrefix = "traceability_core_"): string {
